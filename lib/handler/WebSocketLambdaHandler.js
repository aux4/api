...
class WebSocketLambdaHandler {
  constructor(config, commandPool, options = {}) {
    ...
    const moduleRegistry = options.moduleRegistry || new InProcessHandlerRegistry(config, commandPool);
    this.authHandler = options.authHandler || new AuthHandler(config, commandPool, this.defaultTimeout, moduleRegistry);
