...
class OAuthHandler {
  ...
  async handleCallback(request, reply) {
    ...
    const principal = result.principal && typeof result.principal === "object"
      ? result.principal
      : result;
    ...
    const accessToken = result.accessToken || result.access_token || principal.accessToken || principal.access_token;
    const refreshToken = result.refreshToken || result.refresh_token || principal.refreshToken || principal.refresh_token;
    ...
    const sessionJwt = SessionToken.seal(claims, this.sessionSecret, this.sessionTtl);
