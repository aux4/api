    ...
    for (const [wsPath, routeConfig] of Object.entries(this.wsRoutes)) {
      ...
      app.get(wsPath, { websocket: true }, (socket, request) => {
        ...
        if (routes.$connect) {
          ...
          Command.execute(routes.$connect, JSON.stringify(event), this.defaultTimeout).catch(() => {});
        }
        ...
        socket.on("message", async (data) => {
          ...
          try {
            const { exitCode, stdout } = await Command.execute(command, JSON.stringify(event), this.defaultTimeout);
        ...
        socket.on("close", () => {
          if (routes.$disconnect) {
            ...
            Command.execute(routes.$disconnect, JSON.stringify(event), this.defaultTimeout).catch(() => {});
          }
