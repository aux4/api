    ...
    for (const [wsPath, routeConfig] of Object.entries(this.wsRoutes)) {
      ...
      app.get(wsPath, { websocket: true }, (socket, request) => {
        ...
        if (routes.$connect) {
          ...
          Command.execute(`aux4 ${routes.$connect}`, JSON.stringify(event)).catch(() => {});
        }
        ...
        socket.on("message", async (data) => {
          ...
          try {
            const { stdout } = await Command.execute(`aux4 ${command}`, JSON.stringify(event));
            ...
            if (stdout.trim()) {
              ...
              try {
                ...
                response = { body: stdout.trim() };
              }

              if (response.body) {
                socket.send(typeof response.body === "string" ? response.body : JSON.stringify(response.body));
              }
            }
          } catch {
        ...
        socket.on("close", () => {
          if (routes.$disconnect) {
            ...
            Command.execute(`aux4 ${routes.$disconnect}`, JSON.stringify(event)).catch(() => {});
          }
