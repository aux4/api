...
const { InProcessHandlerRegistry } = require("../InProcessHandlerRegistry");
...
class RestHandler {
  constructor(config, rateLimiter, commandPool, options = {}) {
    this.config = config;
    ...
    this.moduleRegistry = options.moduleRegistry || new InProcessHandlerRegistry(config, commandPool);
    this.authHandler = options.authHandler || new AuthHandler(config, commandPool, this.defaultTimeout, this.moduleRegistry);
  }

  async prepare() {
    if (this.routes.length === 0) this.compile();
    const preparations = [this.authHandler.prepare()];
    for (const route of this.routes) {
      if (route.config.handler) {
        if (route.config.stream) throw new Error("In-process handlers do not support stream routes");
        preparations.push(this.moduleRegistry.prepare(route.config.handler, this.handlerSlot(route)));
      }
    }
  ...
  handlerSlot(route) {
    return `rest:${route.method}:${route.regex.source}`;
  }
  ...
  async handle(request, reply) {
    ...
    if (!principal && this.authHandler.enabled && !route.config.public) {
      const authResult = await this.authHandler.authenticate(request, reply);
      if (authResult.error) {
      ...
      executionEnv = authResult.executionEnv;
    }
    ...
    if (!route.command && !route.config.handler) {
      CookieHandler.clearCookie(reply, route.config.clearCookie);
      if (route.config.redirect) {
        return this.handleRedirect(request, reply, route.config.redirect, principal, executionEnv);
      }
      return reply.status(200).send("{}");
    }

    // Execute command
    const args = route.command ? this.buildArgs(request, pathParameters, event, principal) : "";
    const command = route.command ? (args ? `${route.command} ${args}` : route.command) : "";
    const timeout = route.config.timeout || this.defaultTimeout;

    if (route.config.stream) {
      return this.handleStream(request, reply, command, timeout, executionEnv);
    }

    try {
      const { exitCode, stdout, stderr } = route.config.handler
        ? await this.moduleRegistry.execute(
          route.config.handler,
          this.handlerSlot(route),
          request,
          pathParameters,
          event,
          principal,
        ...
        : await this.commandPool.execute(command, JSON.stringify(event), timeout, executionEnv);
      ...
      if (route.config.redirect) {
        ...
        return this.handleRedirect(request, reply, route.config.redirect, redirectPrincipal, executionEnv);
      }
  ...
  sendResponse(request, reply, output, route) {
    const command = route.command || route.config.handler?.identity || route.config.handler?.package;
    // Data URI
  ...
  async handleRedirect(request, reply, redirectPath, principal, executionEnv) {
    // Try API route
    ...
    if (redirectResult) {
      ...
      const { exitCode, stdout } = await this.commandPool.execute(redirectCommand, null, this.defaultTimeout, executionEnv);
  ...
  handleStream(request, reply, command, timeout, executionEnv) {
    let child;
    try {
      child = this.commandPool.stream(command, null, timeout, executionEnv);
    } catch (error) {
