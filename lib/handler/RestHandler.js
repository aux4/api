...
class RestHandler {
  ...
  async handle(request, reply) {
    ...
    try {
      ...
      return this.sendResponse(request, reply, stdout.trim(), route.command);
    } catch (error) {
  ...
  getComponentPaths(command) {
    // "aux4 contacts list" → { apiPath: "/api/contacts", basePath: "/contacts" }
  ...
  sendResponse(request, reply, output, command) {
    // Data URI
    ...
    if (request.method === "GET" && !prefersJson(request) && hasViews()) {
      const { apiPath, basePath } = this.getComponentPaths(command);
      const html = renderCommandPartial(command, response, apiPath, basePath);
  ...
  sendDataUri(reply, output) {
    ...
    for (const part of parts) {
      if (part.startsWith("filename=")) {
        reply.header("Content-Disposition", `inline; filename="${part.substring(9)}"`);
      }
    }
      ...
      if (exitCode === 0) {
        ...
        try {
          ...
          if (!prefersJson(request) && hasViews()) {
            const { apiPath, basePath } = this.getComponentPaths(redirectResult.route.command);
            const html = renderCommandPartial(redirectResult.route.command, response, apiPath, basePath);
            if (html) return reply.type("text/html").send(html);
          }
          return reply.status(200).type("application/json").send(JSON.stringify(response));
        } catch {
