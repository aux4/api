...
const { resolveBasePath, stripBasePath } = require("../basePath");
...
class RestHandler {
  constructor(config, rateLimiter, commandPool) {
    this.config = config;
    // Path of the config file this app was started with, if known. Forwarded to
    // route commands as --configFile so they can read the same app config (e.g.
    // via `aux4 config get`) without relying on the process CWD — which differs
    // between `api start` (run from the project dir) and `api handle` (Lambda).
    this.configFile = config._configFile || null;
    // Mount prefix for declared routes (default "/api"). "" means ROOT MOUNT —
    // routes are served at bare paths. See lib/basePath.js.
    this.basePath = resolveBasePath(config);
  ...
  async dispatch(event) {
    ...
    const request = buildSyntheticRequest(event, this.basePath);
    ...
    const apiPath = stripBasePath(request.url.replace(/\?.*$/, ""), this.basePath);
  ...
  register(app) {
    ...
    // Under a normal prefix ("/api") mount every declared route below it ("/api/*").
    // Under ROOT MOUNT ("") mount a bare catch-all ("/*"): Fastify's router gives
    // more-specific routes (static "/static/*", views, OAuth "/auth/*", ws) priority
    // over this wildcard, so they are never swallowed; only paths no other handler
    // owns reach handle().
    //
    // OPTIONS is included only for a prefixed mount. At root ("/*") @fastify/cors has
    // already registered its own "/*" OPTIONS preflight route, so declaring OPTIONS
    // here too collides ("Method 'OPTIONS' already declared for route '/*'"). Letting
    // cors own preflight at root is the CORS-correct behavior; a prefixed mount
    // ("/api/*") does not collide with the cors "/*" route, so it keeps OPTIONS
    // exactly as before.
    const methods = ["GET", "POST", "PUT", "DELETE", "PATCH"];
    if (this.basePath) methods.push("OPTIONS");
  ...
  async handle(request, reply) {
    ...
    const apiPath = stripBasePath(urlPath, this.basePath);

    const result = this.match(request.method, apiPath);
    if (!result) {
      // ROOT MOUNT: this handler owns "/*", so an unmatched path would otherwise
      // shadow the app's not-found handler (the views index.hbs SPA fallback, or the
      // default JSON 404). Delegate to it instead of answering here. On the Fastify
      // path reply.callNotFound() exists; the synthetic (lambda/handle) reply has no
      // such method and no views, so fall through to the JSON 404 there.
      if (this.basePath === "" && typeof reply.callNotFound === "function") {
    ...
    // Authentication
    let principal = null;
    // An API Gateway custom authorizer (e.g. the aux4.cloud SSO authorizer) has
    // already validated the caller at the edge; buildSyntheticRequest carried its
    // identity onto request.authorizer. Surface it as the principal so the route
    // command knows who called, without re-running the app auth handler.
    if (request.authorizer && (request.authorizer.sub || request.authorizer.principalId)) {
      principal = request.authorizer;
    }
    if (!principal && this.authHandler.enabled && !route.config.public) {
      const authResult = await this.authHandler.authenticate(request);
      if (authResult.error) {
        // status defaults to 401 (unauthenticated -> login redirect); a scope gate
        // returns 403 (authenticated but not entitled -> no redirect loop).
    ...
    const event = buildRestEvent(request, pathParameters, this.basePath);
  ...
  async handleRedirect(request, reply, redirectPath, principal) {
    ...
    if (redirectResult) {
      const redirectArgs = this.buildArgs(request, redirectResult.pathParameters, buildRestEvent(request, redirectResult.pathParameters, this.basePath), principal);
  ...
  buildArgs(request, pathParameters, event, principal) {
    ...
    // Forward the app's config file so the route command can read shared config
    // (contact/ses/etc.) via `aux4 config get` regardless of the process CWD.
    if (this.configFile) {
      args.push("--configFile", `'${escapeShellArg(this.configFile)}'`);
    }
