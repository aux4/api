...
class RestHandler {
  ...
  async handle(request, reply) {
    ...
    // Auth command check
    let principal = null;
    if (security.auth?.command && !route.config.public) {
      const headersJson = JSON.stringify(request.headers);
      const authCommand = `${security.auth.command} --headers '${headersJson.replace(/'/g, "'\\''")}'`;
      try {
        const { exitCode, stdout } = await this.commandPool.execute(
          authCommand,
          null,
          this.defaultTimeout
        );
    ...
    if (route.config.stream) {
      return this.handleStream(request, reply, route, event, timeout, command);
    }
  ...
  handleStream(request, reply, route, event, timeout, command) {
    let child;
    try {
      child = this.commandPool.stream(command, JSON.stringify(event), timeout);
    } catch (error) {
  ...
  buildArgs(request, pathParameters, event, principal) {
    ...
    if (pathParameters && Object.keys(pathParameters).length > 0) {
      const paramsJson = JSON.stringify(pathParameters);
      args.push(`--params`, `'${paramsJson.replace(/'/g, "'\\''")}'`);
    }
    ...
    if (Object.keys(queryParams).length > 0) {
      ...
      args.push(`--query`, `'${queryJson.replace(/'/g, "'\\''")}'`);
    }

    if (request.headers && Object.keys(request.headers).length > 0) {
      const headersJson = JSON.stringify(request.headers);
      args.push(`--headers`, `'${headersJson.replace(/'/g, "'\\''")}'`);
    }

    if (principal) {
      const principalJson = JSON.stringify(principal);
      args.push(`--principal`, `'${principalJson.replace(/'/g, "'\\''")}'`);
    }

    if (event.body) {
      args.push(`--body`, `'${event.body.replace(/'/g, "'\\''")}'`);
    }
