...
function getHandlebars() {
  if (!_handlebars) {
    ...
    // Built-in helpers
    _handlebars.registerHelper("eq", function (a, b) { return a === b; });
    _handlebars.registerHelper("ne", function (a, b) { return a !== b; });
    _handlebars.registerHelper("fileSize", function (bytes) {
    ...
    const helpersDir = path.resolve("./helpers");
    if (fs.existsSync(helpersDir)) {
      const files = fs.readdirSync(helpersDir);
      for (const file of files) {
        if (file.endsWith(".js")) {
          const name = file.slice(0, -3);
          const fn = require(path.join(helpersDir, file));
          if (typeof fn === "function") {
            _handlebars.registerHelper(name, fn);
          }
        }
      }
    }
