...
function buildSyntheticRequest(event, basePath = "/api") {
  const headers = lowercaseHeaders(event.headers);
  const method = (event.httpMethod || "GET").toUpperCase();
  const path = event.path || "/";
  // Re-attach the mount prefix so handle() strips the same prefix it does on the
  // Fastify path. Root mount ("") leaves the bare path untouched.
  const url = basePath + path + buildQueryString(event);
  ...
  return {
    ...
    // Identity from an API Gateway custom authorizer (e.g. the aux4.cloud SSO
    // authorizer). Carried on the request so handle() can surface it as the
    // route command's principal — the original event is not available there.
