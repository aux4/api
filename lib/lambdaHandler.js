...
async function getExecutionAuthorization(executionId) {
  ...
  if (!response.ok) throw new Error(`Cloud execution authorization failed (${response.status})`);
  const body = await response.json();
  if (!body.accessToken || !body.command) {
...
async function completeExecution(executionId) {
  ...
  await fetch(`${baseUrl}/v1/executions/${encodeURIComponent(executionId)}/complete`, {
    method: "POST",
    headers: { Authorization: `Bearer ${machineToken}` }
  });
}

function runExecutionHook(phase) {
  ...
  if (command) execSync(command, { stdio: "ignore" });
}

async function executeExecutionEvent(event) {
  const executionId = String(event.executionId || "");
  if (!executionId) throw new Error("Execution event is missing executionId");
  const authorization = await getExecutionAuthorization(executionId);
...
async function handleExecutionEvent(event, options = {}) {
  ...
  let response;
  try {
    response = await executeExecutionEvent(event);
  } finally {
    // Structured workflows may resume immediately on another warm Lambda
    // container. Make the session checkpoint durable before acknowledging this
    // phase; otherwise the next phase can observe missing/stale local state.
    await runHook("post");
  }

  if (response && response.status === "final") {
    await completeExecution(String(event.executionId)).catch(() => {});
  }
  return response;
}
