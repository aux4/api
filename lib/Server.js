...
class Server {
  ...
  async start() {
    ...
    app.addHook('onSend', async (request, reply, payload) => {
      ...
      // Skip if request explicitly prefers JSON (for API routes)
      ...
      if (reply.statusCode >= 400 && !reply.getHeader('content-type')?.includes('text/html')) {
        // Format JSON error response for API requests
        if (prefersJson && isApiRoute) {
          let errorMessage = 'An error occurred';
          
          try {
            // Try to parse existing payload to extract error message
            const parsed = JSON.parse(payload);
            errorMessage = parsed.message || parsed.error || errorMessage;
          } catch {
            // If not JSON, use payload as message
