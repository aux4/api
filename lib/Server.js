...
const { resolveBasePath } = require("./basePath");
...
class Server {
  ...
  async start() {
    ...
    // Mount prefix for declared REST routes (default "/api"). "" means ROOT MOUNT:
    // declared routes are served at bare paths. Every place that used to assume the
    // literal "/api" keys off this. See lib/basePath.js.
    const basePath = resolveBasePath(this.config);
    // The "/"-terminated prefix that identifies an API-owned path ("/api/"), or null
    // under root mount where there is no distinguishing prefix (every path is API).
    const apiPrefix = basePath ? basePath + "/" : null;
    ...
    // Tolerant application/json parser. aux4/api is an HTTP -> CLI bridge: it must be able to
    // route (and let a mounted command record) every request, including an empty-body POST/PATCH
    // that carries Content-Type: application/json — a shape real APIs accept (e.g. Google
    // Calendar quickAdd, where the payload rides in the query string). Fastify's default JSON
    // parser rejects an empty body with 400 FST_ERR_CTP_EMPTY_JSON_BODY BEFORE the route handler
    // runs, so the request is never seen. Here an empty body passes through as no body; a
    // non-empty body is parsed exactly as before (valid JSON -> object; invalid JSON -> 400).
    // Non-empty bodies are parsed with secure-json-parse using the same protoAction/
    // constructorAction: "remove" options as Fastify's default parser, so __proto__ /
    // constructor prototype-pollution hardening is preserved — this change is strictly
    // additive (empty-body tolerance) with no security regression.
    app.addContentTypeParser("application/json", { parseAs: "string" }, (req, body, done) => {
      ...
      try {
        done(null, secureJson.parse(body, { protoAction: "remove", constructorAction: "remove" }));
      } catch (err) {
        err.statusCode = 400;
        done(err);
      }
    ...
    // Catch-all parser for any content-type without a dedicated parser: multipart/related
    // (e.g. from `aux4 curl --upload`), application/octet-stream, a raw byte PUT, or a request
    // with NO Content-Type header. Without this, Fastify returns 415
    // FST_ERR_CTP_INVALID_MEDIA_TYPE at the content-type layer BEFORE the route runs, so a
    // mounted command (and thus aux4/mock) never sees the request. The raw body is captured as
    // a UTF-8 string and flows to event.body unchanged (isBase64Encoded stays false), so the
    // command can record and match it. The registered types (application/json, text/plain,
    // application/x-www-form-urlencoded, multipart/form-data) keep their own parsers and are
    // unaffected — a specific parser always wins over this wildcard.
    ...
    const ipHook = ipAllowlist(this.config.security, basePath);
    if (ipHook) {
    ...
    // OAuth mode: send unauthenticated browsers to the login redirect on 401.
    if (this.config.security?.auth?.type === "oauth" && !errorRedirects["401"]) {
      const oauthRedirect = this.config.security.auth.redirectOnError || "/auth/signin";
      errorRedirects["401"] = oauthRedirect;
    }
    ...
    app.addHook("onSend", async (request, reply, payload) => {
      ...
      if (prefersJson && (apiPrefix === null || request.url.startsWith(apiPrefix))) {
  ...
  class Aux4Component extends HTMLElement {
    connectedCallback() {
      ...
      var apiPath = p.startsWith(${JSON.stringify(apiPrefix || "/")}) ? p : ${JSON.stringify(basePath)} + p;
  ...
  customElements.define("aux4-component", Aux4Component);
      ...
      app.post("/aux4/batch", async (request, reply) => {
        ...
        if (urls.some(u => typeof u !== "string" || !u.startsWith(apiPrefix || "/"))) return reply.status(400).send({ error: "invalid urls" });
    ...
    // OAuth web-login routes (security.auth.type: oauth)
    const oauthHandler = new OAuthHandler(this.config, this.config.server?.timeout || 30000);
    if (oauthHandler.enabled) {
      oauthHandler.register(app);
    }
    ...
    app.setNotFoundHandler(async (request, reply) => {
      const notApiPath = apiPrefix === null || !request.url.startsWith(apiPrefix);
      if (request.method === "GET" && fs.existsSync("./views/index.hbs") && notApiPath) {
    ...
    // Adapter path: hand back the fully-built app (all plugins + routes, incl.
    // @fastify/static) WITHOUT opening a socket, so it can be wrapped by
    // @fastify/aws-lambda and driven by API-Gateway proxy events. Used by the
    // `lambda` runtime; `start()` otherwise listens as before.
    if (this.config._returnApp) {
      // Return NOT-yet-ready: @fastify/aws-lambda must add its `awsLambda`
      // decorator before ready(), and it drives readiness on first invocation.
      return app;
    }
