...
class Server {
  ...
  async start() {
    ...
    if (hasComponents) {
      ...
      const componentJs = `class Aux4Component extends HTMLElement {
  async connectedCallback() {
    ...
    if (this.getAttribute("route") === "true") {
      path = window.location.pathname.startsWith(src) ? window.location.pathname : src;
    }
    const apiPath = path.startsWith("/api/") ? path : "/api" + path;
    const url = query ? apiPath + "?" + query : apiPath;
    const res = await fetch(url, { headers: { Accept: "text/html" } });
    if (res.ok) {
      if (res.headers.get("X-Auth-Redirect") === "true") {
        var app = document.getElementById("app");
        if (app) { app.innerHTML = await res.text(); app.querySelectorAll("script").forEach(function(s) { var n = document.createElement("script"); n.textContent = s.textContent; s.replaceWith(n); }); if (typeof htmx !== "undefined") htmx.process(app); return; }
      }
      this.innerHTML = await res.text();
      this.querySelectorAll("script").forEach(function(s) { var n = document.createElement("script"); n.textContent = s.textContent; s.replaceWith(n); });
